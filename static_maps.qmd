...
Since a few years, it is possible to make really attractive maps using only R. For example, almost all the maps of the Swiss Breeding Bird Atlas 2013-16 were produced in R. Standard GIS software will obviously provide more advanced cartographic tools, such as advanced labeling and masking. Nonetheless R is good enough for most cases. Moreover you definitely have a big advantage in terms of reproducibility. In this part we will first look at how we can use `sf` and `terra` standard plotting functions to make simple maps. Then we'll use the `tmap` package which massively simplify the creation of maps and also provides interesting tools to produce complex layouts.
...
As we've seen in some previous examples, the most basic way to make maps is simply to use the `plot()` function. Both `sf` and `terra` extend this function to support vector and raster data sets. Most of the standard arguments provided by the standard `plot()` function (such as `main`, `pch`, `lwd`, `cex`, etc.) are also supported. If you want to see all the additional parameters, you can check the help files (`?sf::plot` and `?terra::plot`).

To plot `sfc` objects, we simply pass the object to the `plot()` function. To plot a specific attribute of an `sf` object, we need to add the name of the attribute inside square brackets. Here's a simple example:

```{r, echo = -(1:2)}
# Optimize margins for Quarto HTML rendering (hidden in the HTML output)
par(mar = c(0, 0, 1, 0))
muni_sfc <- st_geometry(muni)
plot(muni_sfc)
plot(muni["name"], lwd = 2, main = "Municipalities")
```

We can also plot several attributes simultaneously, but this will unfortunately remove the legend and we can't use the `main` argument to add a global title. If you don't specify any attribute, the first 10 will be plotted. Note that `sf` uses by default different color palettes for categorical and continuous variables.

```{r}
#| fig-height: 4
plot(muni[c("name", "popsize")], lwd = 2)
```

Maps of polygons with some continuous attribute are called choropleth maps. These maps are often used to represent spatially variables such as population densities or per capita income. The number of breaks and the algorithm used to compute these breaks have a huge influence on the output map. Choose these settings carefully and be aware that they can dramatically change the interpretation of a map (have a look at <https://exploropleth.github.io/exploropleth>{target="_blank"} to see some examples).

Let's have a look at the population size of all the cantons in Switzerland:
...
If, like me, you're not a big fan of the default colors used by `sf`, you can change the color palette thanks to the `pal` argument. You can use all the palettes available in R. You should especially have a look at the `RColorBrewer` and `viridis` packages. These were designed by color specialists and are highly recommended. For `RColorBrewer` you can visualize the existing color palettes on the following website: <https://colorbrewer2.org>{target="_blank"}. Note that some ColorBrewer and Viridis palettes are already included in the `hcl.colors()` color palette function (to see the full list, use `hcl.pals()`).

Guessing the number of colors you'll need to use inside the color palette function can be a bit tricky (or at least I haven't found a consistent way to make things easier). By default `sf` will create 10 evenly distributed breaks (using the `pretty` algorithm) and you'll thus need 9 colors, but depending on the data and the chosen algorithm, this number can be different. If you're not sure, just create a plot using the default colors and count the number of unique colors.

```{r}
plot(cantons["popsize"], pal = hcl.colors(8, "Blues", rev = TRUE))
```

We can easily change the number of breaks using the `nbreaks` argument. Let's reduce the number of breaks to see if this improves our map.

```{r}
plot(cantons["popsize"], nbreaks = 5, pal = hcl.colors(4, "Blues", rev = TRUE))
```

Sometimes it's better to use another algorithm to define the breaks, we can change this setting using the `breaks` argument. For example the quantile algorithm will ensure that each class has the same number of features (careful, these maps can often be misleading).

```{r}
plot(cantons["popsize"], nbreaks = 5, breaks = "quantile", pal = hcl.colors(5, "Blues", rev = TRUE))
```

Of course it's also possible to define our own breaks by passing a numeric vector with the actual breaks to the `breaks` argument. Don't forget to also use the `at` argument in order to have a matching legend.

```{r}
plot(cantons["popsize"], breaks = c(min(cantons$popsize), 200000, 500000, 1000000, max(cantons$popsize)), at = c(200000, 500000, 1000000), pal = hcl.colors(4, "Blues", rev = TRUE))
```

::: {.callout-note icon="false" title="Exercise (5 minutes)"}
Compute the population density and plot the corresponding choropleth map. Use another algorithm to compute the breaks and change the colors used.
...
When we create maps, we usually consider the different data sets as layers that we combine to produce a nice output. This is why we will often use the `add = TRUE` argument in the `plot()` function.
...
The map looks OK but we're definitely missing a legend for the points. Adding such a legend is a bit tricky, we need to use a lookup table to match the species names and the colors inside the `plot()` function. This is, in my opinion, a good example of why specialized packages like `tmap` should be preferred.

```{r, echo = -(1:2)}
# Optimize margins for Quarto HTML rendering (hidden in the HTML output)
par(mar = c(0, 0, 1, 0))
obsColors <- data.frame(name = unique(obs$name), color = sf.colors(length(unique(obs$name)), categorical = TRUE))
plot(st_geometry(muni), col = "grey90", border = "grey50", lty = 3)
plot(st_union(muni), col = NA, border = "grey50", lwd = 2, add = TRUE)
plot(obs["name"], pch = 16, cex = 2, col = obsColors$color[match(obs$name, obsColors$name)], add = TRUE)
legend(x = "topright", legend = unique(obs$name), col = obsColors$color, pch = 16, bty = "n")
```

::: {.callout-note icon="false" title="Exercise (5 minutes)"}
Try to modify some default settings, change the colors and add a few streets to your map.
:::

Plotting raster data is very similar, again use we use the `plot()` function.

```{r}
plot(elev)
```

We get a continuous legend by default for continuous rasters, but we can easily use intervals with the `breaks` argument. This should be either a single number specifying the number of breaks or a vector with the actual breaks. The color palette can be changed using the `col` argument.

```{r}
plot(elev, breaks = 10, col = hcl.colors(10, "Blues"))
```

And we can of course combine vector and raster data using layers like in the previous example.

```{r}
plot(elev, breaks = 10, col = hcl.colors(10, "Blues"))
plot(st_geometry(muni), border = "white", lwd = 3, col = rgb(1, 1, 1, 0.4), add = TRUE)
```
...
The `plot()` function doesn't understand CRSs. Our initial buffer (using EPSG:2056) will therefore not be placed correctly on the world map (using EPSG:4326). We first need to make sure that all data sets use a common CRS.
...
To help people understanding your maps, you'll often need to add some kind of basemap such as a topographic map or aerial imagery. If you don't have high quality basemaps at your disposal, you'll need to use some online map provider such as [OpenStreetMap](https://www.openstreetmap.org){target="_blank"}. In this section we will see how to use the `maptiles`[@giraud_maptiles_2025] package to do this.

::: callout-important
Creating a basemap and keeping it up to date is a lot of work! Please do not forget to cite it properly when you use some basemap.
:::

When you browse an online map, even though everything looks like a single map, you are actually looking at many small tiles stitched together at a specific zoom level. The `get_tiles()` function allows us to download and merge these tiles. To specify the extent you want to download, you can use an `sf` or `sfc` object (`SpatVector` and `SpatRaster` are also working). The `get_tiles()` function will automatically use the bounding box of the object. You also need to specify the desired provider (use the `get_providers()` function to get a list of the available providers). The output will be a `SpatRaster` object with 3 bands (red, green and blue).

```{r}
library(maptiles)

osm_basemap <- get_tiles(obs_in_sempach, provider = "OpenStreetMap", crop = TRUE)
plot(osm_basemap)
```

I recommend using `crop = TRUE` to get a basemap cropped to the bounding box of your object, otherwise you'll get all the tiles that intersect the bounding box. If you need another zoom level, you can easily get it using the `zoom` argument.

```{r}
osm_basemap <- get_tiles(obs_in_sempach, provider = "OpenStreetMap", crop = TRUE, zoom = 13)
plot(osm_basemap)
```

You've probably noticed that the resolution of the basemap is rather low. One reason is that the map is automatically reprojected to the CRS of the object defining the extent. As we saw earlier, reprojecting rasters imply some sort of resampling that will impact the rendering quality. We can solve this using `project = FALSE` inside the `get_tiles()` function. The CRS of the output raster will then be EPSG:3857 (Pseudo-Mercator).

```{r}
osm_basemap <- get_tiles(obs_in_sempach, provider = "OpenStreetMap", project = FALSE, crop = TRUE)
plot(osm_basemap)
```

The quality is slightly better but we're hitting the main problem of most online map providers, the resolution they use is not high enough for creating high quality maps. Fortunately some providers allow downloading tiles with a better resolution, such as all the basemaps provided by Carto and Stadia (note that you'll need an API key to use Stadia).

```{r}
carto_basemap <- get_tiles(obs_in_sempach, provider = "CartoDB.Voyager", project = FALSE, crop = TRUE)
plot(carto_basemap)
```

If you find the colored basemaps too disturbing, you can easily create black and white versions using the following trick. Note that we need to use `plotRGB()` instead of `plot()` because `terra` doesn't properly recognize the RGB bands.

```{r}
osm_bw = (osm_basemap[[1]] + osm_basemap[[2]] + osm_basemap[[3]])/3
plotRGB(rast(list(osm_bw, osm_bw, osm_bw)))
```

If you want to use another provider that is not included in `maptiles`, you can easily create a custom provider using the `create_provider()` function. Then you can use this new provider inside the `get_tiles()` function. Note that the provider has to provide tiles using EPSG:3857 (it will not work with other CRSs). In the next example, we create 3 providers to get maps and imagery from Swisstopo.

```{r}
# Create a new provider
swisstopo_grey_prov <- create_provider(
    name = "swisstopo_grey",
    url = "https://wmts.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-grau/default/current/3857/{z}/{x}/{y}.jpeg",
    citation = "Background map © Swisstopo")

swisstopo_color_prov <- create_provider(
    name = "swisstopo_color",
    url = "https://wmts.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-farbe/default/current/3857/{z}/{x}/{y}.jpeg",
    citation = "Background map © Swisstopo")

swisstopo_ortho_prov <- create_provider(
    name = "swisstopo_ortho",
    url = "https://wmts.geo.admin.ch/1.0.0/ch.swisstopo.swissimage/default/current/3857/{z}/{x}/{y}.jpeg",
    citation = "Background map © Swisstopo")

swisstopo_grey_basemap <- get_tiles(obs_in_sempach, provider = swisstopo_grey_prov, project = FALSE, crop = TRUE)
plot(swisstopo_grey_basemap)
swisstopo_color_basemap <- get_tiles(obs_in_sempach, provider = swisstopo_color_prov, project = FALSE, crop = TRUE)
plot(swisstopo_color_basemap)
swisstopo_ortho_basemap <- get_tiles(obs_in_sempach, provider = swisstopo_ortho_prov, project = FALSE, crop = TRUE)
plot(swisstopo_ortho_basemap)
```

When using data from Swisstopo downloaded with `maptiles`, you will notice that the grid lines on the topographic maps won't appear as straight perpendicular lines. This is caused by the reprojection of the original maps to EPSG:3857 on the Swisstopo server. The orthophotos also have some strange display artefacts.

We've already seen how to automatically crop the basemaps but sometimes you'll need basemaps with an extent that is slightly bigger than the bounding box of your `sf` object. You can easily do that by using an affine transformation (see @sec-affine_transformations for details). For example let's make the bounding box 10% bigger:

```{r}
bbox_sfc <- st_as_sfc(st_bbox(obs_in_sempach))
expanded <- (bbox_sfc - st_centroid(bbox_sfc)) * 1.1 + st_centroid(bbox_sfc)
st_crs(expanded) <- "EPSG:2056"

swisstopo_grey_basemap <- get_tiles(expanded, provider = swisstopo_grey_prov, project = FALSE, crop = TRUE, zoom = 14)
plot(swisstopo_grey_basemap)
points(st_transform(obs_in_sempach, "EPSG:3857"), col = "blue")
```
...
# Plot
plotRGB(hillshade, alpha = 0.7, smooth = TRUE, mar = c(1, 1, 1, 1), axes = FALSE, legend = FALSE, maxcell = Inf)
...
If your spatial objects are using different CRSs, `tmap` will use the CRS of the object inside the first `tm_shape()` function to draw the map and automatically reproject the data sets used in the following `tm_shape()` functions.
...
We can include extra objects in our map, such as titles, scale bars or north arrows. Each element has a `position` argument. In the next example we also include some styling using the `tm_style()` function. Currently 9 styles are available (plus some old ones from `tmap` version 3). To list the available styles, run the `tmap_style()` function. You can also use the `tmap_style_catalog()` function to create sample maps using all the styles (the maps will be saved as PNG files in your working directory). We finally use the `tm_layout()` function to increase the right margin and draw a frame with sharp corners. For margins (specified as bottom, left, top, right), the units are relative to the map frame, so 0.05 means 5 percent of the frame height. Note that when using `tm_style()`, all the layout options are reset according to the chosen style. Therefore if you need to change additional layout elements, you'll need to call the `tm_layout()` function after `tm_style()`.
...
Facets can even be defined for two or three dimensions using the `tm_facets_grid()` function. Here's how we can use it to create individual maps for two different variables and for each municipality.

```{r}
tm_shape(muni) + tm_polygons(fill = c("popsize", "area")) + tm_facets_grid(columns = "name")
```

### Basemaps

Adding a basemap to your map is extremely easy with `tmap` thanks to the `tm_basemap()` function. The `maptiles` package is used in the background to download the tiles. You only need to specify the provider name inside the `tm_basemap()` function, and you can adjust the zoom level with the `zoom` argument. Use the `tmap_providers()` function to get the list of available providers.

::: callout-important
Creating a basemap and keeping it up to date is a lot of work! Please do not forget to cite it properly when you use some basemap.

Adding basemaps to static maps will work only with version 4.1 (or newer) of `tmap`.
:::

```{r}
map <- tm_shape(obs_in_sempach) + tm_dots(size = 0.5, fill = "red")
map + tm_basemap("OpenStreetMap")
map + tm_basemap("OpenStreetMap", zoom = 13)
map + tm_basemap("CartoDB.Voyager")
```

If you want to use another provider that is not included by default, you can directly use the URL of the tile server inside the `tm_basemap()` function. Note that the provider has to provide tiles using EPSG:3857 (it will not work with other CRSs). In the next example, we create maps combining bird sightings, topographic maps and imagery from Swisstopo.

```{r}
map + tm_basemap(server = "https://wmts.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-grau/default/current/3857/{z}/{x}/{y}.jpeg")
map + tm_basemap(server = "https://wmts.geo.admin.ch/1.0.0/ch.swisstopo.pixelkarte-farbe/default/current/3857/{z}/{x}/{y}.jpeg")
map + tm_basemap(server = "https://wmts.geo.admin.ch/1.0.0/ch.swisstopo.swissimage/default/current/3857/{z}/{x}/{y}.jpeg")
```

When using data from Swisstopo with `tmap`, you will notice that the grid lines on the topographic maps won't appear as straight perpendicular lines. This is caused by the reprojection of the original maps to EPSG:3857 on the Swisstopo server. The orthophotos also have some strange display artefacts.
...
You can also use the `tm_style("natural")` function to specify a style adapted for world maps (remember to call `tm_layout()` after and not before `tm_style()`). Here's an example using the Robinson projection.
...
tm_crs(projstring, bbox = "FULL") +
tm_layout(bg.color = "lightblue", earth_boundary = TRUE) +
