...
The main function used to create maps is simply called `mapview()`. It will accept any `sf` or `sfc` object. When mapping an `sf` object, you can easily check the related attributes by clicking on the features.
...
If you don't like the default color, you can change it using the `color.regions` argument. It is also possible to specify the name of an attribute that will be shown on mouseover using the `label` argument. Here we will use the name of the municipalities. We also change the name of the data set that is shown on the map using the `layer.name` argument.

```{r}
#| classes: map-padding
mapview(muni, col.regions = "purple", label = "name", layer.name = "Municipalities")
```
...
If you want to produce a map for a specific data set but use the extent of another data set, you can use the `hide = TRUE` argument for the data set defining the extent. Note that the hidden layer will still be available in the layer list.

```{r}
#| classes: map-padding
mapview(muni[muni$name == "Sursee",]) + mapview(muni, hide = TRUE)
```
