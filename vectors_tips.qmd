...
::: callout-note
The `lwgeom` package is required in order to use the `st_perimeter()` function.
:::
...
The S2 library performs its computations on a sphere. For areas and lengths, it is possible to obtain a better accuracy by using an ellipsoid. You can do this by turning off the S2 library with the `sf_use_s2(FALSE)` function. In this case, `sf` will automatically use equivalent functions provided by the `lwgeom`[@pebesma_lwgeom_2023] package. These functions use algorithms from the [GeographicLib](https://geographiclib.sourceforge.io){target="_blank"} library which are the most precise ones you can currently get if you need to compute measurements on an ellipsoid. Don't forget to reactivate S2 (using the `sf_use_s2(TRUE)` function) when you're done.
...
We will now make a short excursion in the world of geographic CRSs with a quick example showing what `sf` is doing when we don't use standard Euclidean geometry. We will compute the distance between the centroids of Switzerland and Australia. By default `sf` is using the S2 library to perform its computations on a sphere when we use a geographic CRS. However for distances, it is possible to obtain a better accuracy by using an ellipsoid. To do this, we need to tell `sf` to avoid using S2 with the function `sf_use_s2(FALSE)`. The `st_distance()` will thus be forced to use a more precise algorithm (by automatically using a similar function from the `lwgeom` package). We can of course compare the two estimates.
...
par(mfrow = c(1, 3))
plot(line, col = "navy", lwd = 2, main = "Line direction")
arrows(st_coordinates(line)[2,1], st_coordinates(line)[2,2], st_coordinates(line)[3,1], st_coordinates(line)[3,2], length = 0.15, col = "navy", lwd = 2)
plot(line_singlebuff1, main = "Left-sided")
plot(line, col = "navy", lwd = 2, add = TRUE)
plot(line_singlebuff2, main = "Right-sided")
plot(line, col = "navy", lwd = 2, add = TRUE)
...
## Affine transformations {#sec-affine_transformations}
...
## CRS transformations {#sec-vector_crs_transformations}
...
If the data sets you're using have different CRSs, it's usually a good idea to transform some of them so that they all have the same CRS. When using a projected CRS, we can also say that we project the data to another CRS. However, transformations between geographic CRSs are not projections. The `st_transform()` function is the only one we need to perform all the available CRS transformations.

The first argument of `st_transform()` is the object (`sf` or `sfc`) we want to transform, the second argument is the CRS we want to use. The CRS is specified using the authority:identifier form. If we use a CRS with an EPSG number, we can also use the integer value directly (without specifying "EPSG:"). However it's a good habit to always specify the authority to avoid confusions. As a first example, let's check the CRS of our bird sightings and then perform a CRS transformation to EPSG:4326 (WGS 84).

```{r}
st_crs(obs)$Name
obs_wgs <- st_transform(obs, "EPSG:4326")
obs_wgs
```

We see that our `sf` object is now using the chosen geographic CRS, and the geometry column was modified (the coordinates are now in degrees).

In the next example we first compute a graticule (i.e. a grid). Then we project the `World` data set with the associated graticule using three different projected CRSs, the Swiss CRS, the Equal Earth projection and the Robinson projection.
...
When we plot that data, we first see that, not surprisingly, the Swiss CRS is not adapted at all for global data, and that the Equal Earth and Robinson projections both give pleasing results. These two projections are good choices for world maps.
...
par(mar = c(1, 2, 1, 2))
par(mfrow = c(2, 2))
plot(st_geometry(World), col = "grey90", main = "WGS84 (plate carrée)")
plot(st_geometry(grat), col = "lightgrey", add = TRUE)
plot(st_geometry(world_ch), col = "grey90", main = "Swiss projection")
plot(st_geometry(grat_ch), col = "lightgrey", add = TRUE)
plot(st_geometry(world_equal), col = "grey90", main = "Equal Earth projection")
plot(st_geometry(grat_equal), col = "lightgrey", add = TRUE)
plot(st_geometry(world_rob), col = "grey90", main = "Robinson projection")
...
We've already seen that the Pseudo-Mercator projection (EPSG:3857) used by most online maps should be avoided at all costs when computing measurements such as areas or distances. As an example, let's compute the "true" area of Greenland using a geographic CRS, first with the most precise approximation and then using S2. After that we compare these areas with the estimated area after projecting Greenland to Pseudo-Mercator.

```{r}
greenland <- World[World$name == "Greenland",]

sf_use_s2(FALSE)
a1 <- st_area(greenland)
sf_use_s2(TRUE)
a2 <- st_area(greenland)
a3 <- st_area(st_transform(greenland, "EPSG:3857"))

units::set_units(c(a1, a2, a3), "km^2")
```

When projected using the Pseudo-Mercator projection, the estimated area of Greenland is sixteen times larger than its actual area...

To see how bad distance estimations are when using Pseudo-Mercator, let's compute the "true" distance between the Swiss Ornithological Institute and Cape Town using a geographic CRS, first with the most precise approximation and then using S2. After that we compare these distances with the estimated distance after projecting both points to Pseudo-Mercator.

```{r}
soi_4326 <- st_transform(soi, "EPSG:4326")
capetown_4326 <- st_as_sfc("POINT(18.42 -33.91)", crs = "EPSG:4326")

soi_3857 <- st_transform(soi_4326, "EPSG:3857")
capetown_3857 <- st_transform(capetown_4326, "EPSG:3857")

sf_use_s2(FALSE)
d1 <- st_distance(soi_4326, capetown_4326)
sf_use_s2(TRUE)
d2 <- st_distance(soi_4326, capetown_4326)
d3 <- st_distance(soi_3857, capetown_3857)

units::set_units(c(d1, d2, d3), "km")
```
 
Once again, the results obtained when using the Pseudo-Mercator projection are completely wrong.

In the chapter on coordinate reference systems, we discovered that there are often several possible transformation pipelines when transforming data between two different CRSs. The PROJ library is always trying to automatically use the best available pipeline. If for some reason you need to perform a CRS transformation using another pipeline than the one automatically chosen by PROJ, you can specify the desired pipeline inside the `st_transform()` function. You first need to call the `sf_proj_pipelines()` function to get a list of the available pipelines. You can then use the desired pipeline definition inside the `st_transform()` function using the `pipeline` argument. In the next example we see how to use the two different pipelines available to transform data from the old and deprecated Swiss CRS (EPSG:21781) to the current one (EPSG:2056).

```{r}
pt <- st_as_sfc("POINT(500000 118000)", crs = "EPSG:21781")

sf_proj_network(enable = TRUE)
pipe <- sf_proj_pipelines(source_crs = "EPSG:21781", target_crs = "EPSG:2056")
pipe$definition

st_coordinates(st_transform(pt, "EPSG:2056", pipeline = pipe$definition[1]))
st_coordinates(st_transform(pt, "EPSG:2056", pipeline = pipe$definition[2]))
```

We see that there is a difference of 1 meter for the *x* coordinate between the first pipeline (the most accurate) and the second one.