...
If the data sets you're using have different CRSs, it's usually a good idea to transform some of them so that they all have the same CRS. We need the `st_transform()` function to do this. When using a projected CRS, we can also say that we project the data to another CRS. However, transformations between geographic CRSs are not projections.
...
When we plot that data, we first see that, not surprisingly, the Swiss CRS is not adapted at all for global data, and that the Equal Earth and Robinson projections both give pleasing results. Both of these projections are good choices for world maps.
