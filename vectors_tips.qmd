...
The S2 library performs its computations on a sphere. For areas and lengths, it is possible to obtain a better approximation by using an ellipsoid. You can do this by turning off the S2 library with the `sf_use_s2(FALSE)` function. In this case, `sf` will automatically use equivalent functions provided by the `lwgeom`[@pebesma_lwgeom_2023] package. These functions use algorithms from the [GeographicLib](https://geographiclib.sourceforge.io){target="_blank"} library which are to my knowledge the most precise approximations you can currently get. Don't forget to reactivate S2 (using the `sf_use_s2(TRUE)` function) when you're done.
